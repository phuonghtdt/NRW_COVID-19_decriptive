...
[Google Community Mobility Reports](https://www.google.com/covid19/mobility/) shows how a community is moving around due to COVID-19 using individual movement histories to provide location-specific (retail and recreation, grocery and pharmacy, workplaces, transit stations, residential and parks) movement trends over time.
...
[Data & description:](https://research.facebook.com/blog/2020/06/protecting-privacy-in-facebook-mobility-data-during-the-covid-19-response/)
...
#1 COVIMOD
##########################
dummy <- df # df data from sumary table chunk
p1 <- ggplot(dummy, aes(x=wave_date, y=all_contact_n))+ 
    stat_summary(geom="ribbon", fun.data=mean_cl_normal, 
                 fun.args=list(conf.int=0.95), fill="lightblue")+
    stat_summary(geom="line", fun=mean, linetype="dashed")+
    stat_summary(geom="point", fun=mean, color="red")+
  coord_cartesian( ylim = c(0, 6))+
   ggtitle("COVIMOD survey")+
  labs(y="Mean_contact",x=" ")+
  scale_x_date(breaks = unique(df$wave_date))+ # display all dates on x axis
  scale_x_date(date_labels="%Y-%m-%d", date_breaks = "14 days", 
 ...
 theme_classic()+
  theme(axis.text.x = element_text(colour = "black", size = 10, angle = 90, hjust = 0.5, vjust = 0,5),
        ...
        # axis.ticks.length.y = unit(0.85,"cm"),#distance figure from y axis
...
dummy <- mobi # as original data
p2 <- ggplot(dummy,aes( x = date)) +
   geom_line( aes(y = cx,color = "cx"), size = 1)+
   geom_line( aes(y = cx_mean7da,color = "cx_mean7da"), size = 1)+
  
   labs(y="Contact index",x="")+
   # ylim (0,1000)+
   ggtitle("Netcheck" )+
   # scale_x_date(breaks = "21 days")+ # display all dates on x axis
  scale_x_date(date_labels="%Y-%m-%d", date_breaks = "14 days", 
...
####################################
# Movement trend (google and facebook)
####################################

dummy <- google
dummy <- merge(dummy, facebook, by = "date", all="T") ## add facebook data


p3 <- ggplot(dummy,aes( x = date)) +
  geom_line( aes(y = ave_allplace_nopark_mean7da,color = "ave_allplace_nopark_mean7da"), size = 1.5)+
  geom_line( aes(y = grocery_pharmac_mean7da,color = "grocery_pharmac_mean7da"), size = 1)+
  geom_line( aes(y = retail_recreation_mean7da,color = "retail_recreation_mean7da"), size = 1)+
  geom_line( aes(y = transit_stations_mean7da,color = "transit_stations_mean7da"), size = 1)+
  geom_line( aes(y = workplaces_mean7da,color = "workplaces_mean7da"), size = 1)+
  geom_line( aes(y = residential_mean7da,color = "residential_mean7da"), size = 1)+
  geom_line( aes(y = movement_change_mean7da*100,color = "movement_change_mean7da"), size = 1.5)+ #facebook
  geom_line( aes(y = prop_one_location_mean7da*100,color = "prop_one_location_mean7da"), size = 1.5)+ #facebook
   geom_hline(yintercept=0, linetype="dashed", 
                color = "black", size=1)+
   labs(y="",x="", color = "")+
   ggtitle(" Google and facebook" )+
   scale_y_continuous(limits = c(-100,100),breaks = seq(-100,100, by = 50),
                      labels = c("-100%","-50%","Baseline","50%","100%"))+ #labels = unit_format(unit ="%")
  # scale_x_date(breaks = "21 days")+ # display all dates on x axis
  scale_x_date(date_labels="%Y-%m-%d", date_breaks = "14 days", 
               limits = c(as.Date("2020-03-01"),as.Date("2021-12-31")),
               expand = c(0, 0))+
  scale_color_manual(name = "", values = c("ave_allplace_nopark_mean7da"="#d73027",
                                           "grocery_pharmac_mean7da"="#f46d43",
                                           "retail_recreation_mean7da"="#fdae61",
                                           "transit_stations_mean7da"="#fee090",
                                           "workplaces_mean7da"="#8c510a",
                                           "residential_mean7da"="#01665e",
                                           "prop_one_location_mean7da"= "#4d9221",
                                           "movement_change_mean7da"="#542788"
                                           ),
                     
                     labels = c("Average change (google)",
                          "Grocery_pharmacy (google)",
                          "Retail_recreation (google)",
                          "Transit_stations(google)",
                          "Workplaces(google)",
                          "Residential(google)",
                           "Stay a single place (facebook)",
                          "Change in movement (facebook)"))+
   theme_classic()+
  theme(axis.text.x = element_text(colour = "black", size = 10, angle = 90, hjust = 0.5, vjust = 0,5),
        axis.text.y = element_text(colour = "black", size = 12, face = "bold"),
        axis.title = element_text(colour = "black", size = 12, face = "bold"),
