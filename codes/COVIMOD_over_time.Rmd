...
#age_0 with NAs value => was imputed into agegroup without NAs?
dummynames<-c("new_id", "wave","age_group_participant","age_participant")
...
dummy$place_oth[is.na(dummy$place_oth)] <- 0 #change to 0 ## = No because no place was indicated?

# a specific place can take "yes", "no" and NA, now change to "yes"=1, "no"=0
...
###remove <15 yrs
Age refers to the age of the working person, therefore people <15 out
...
###replace NA
replaces NA by 0(No) => is it reasonable?
Need motivation for doing this?
...
colnames(dummy) <- dummynames
dummy <- merge(dummy, dt_hh_all, by=c("new_id", "wave"), all.y=T) # is enough specificity level ???

# dummy$place_oth <- as.factor(dummy$place_oth) #change character to factor##why?
...
```{r merge households}
# # Household sizes
# d_self_all <- d_self_all %>% mutate(hh_p_incl_0 = ifelse(wave > 0, (hh_p_incl_0+1), hh_p_incl_0)) # why +1
# 
# #reorder
...
- data type: factor(almost) and character(place_oth)
- value:[0,1] (almost) and [yes,no] (place_dk)

```{r Count multiple contacts}

# # variable is presented in the dataset, no need to calculate again. so, the code below is refrence when needed
# dt_hh_all <- dt_hh_all %>% dplyr::mutate(place_multiple = rowSums(select(.,place_home_own:place_oth) == 1, na.rm=TRUE))

dt_hh_all <- dt_hh_all %>% dplyr::mutate(place_multiple = rowSums(select(.,place_home_own:place_oth) == 1, na.rm=TRUE)) #delete this code may cause error in te next chunk?

table(dt_hh_all$place_multiple)

# dt_hh_all$place_multiple <- as.integer(dt_hh_all$place_multiple >= 2) # sum of at least 2 places

dt_outside_all <- dt_outside_all %>% dplyr::mutate(place_multiple = rowSums(select(.,place_home_own:place_oth) == 1, na.rm=TRUE))
...
warning: change all.x=T instead of all =T where merge function was used
...
# #one obs(new_id) per wave (wave) may have both hh_contacts and outside_contacts => merged data can be larger than either original files

d_all_wide <- merge(d_self_all, hh_contacts, by=c("new_id", "wave"), all.x=T) #all.x: more variable but less row
...
# Inside contacts
aux_hh <- dt_hh_all %>% filter(place_inside==1) #1 = Yes, 0=No
hh_contacts <- hh_contact_number(aux_hh, new_id, wave)

aux_outside <- dt_outside_all %>% filter(place_inside==0)
...
# Outside contacts
aux_hh <- dt_hh_all %>% filter(place_outside==1)#1 = Yes, 0=No
hh_contacts <- hh_contact_number(aux_hh, new_id, wave)
