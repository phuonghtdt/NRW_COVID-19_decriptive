...
# also add to all non_hh_
d_all_wide$non_hh_contact_n_lshtm_ <- d_all_wide$non_hh_contact_n + 
  d_all_wide$Q75_combined

# also add to all_
d_all_wide$all_contact_n_lshtm <- d_all_wide$all_contact_n + 
  d_all_wide$Q75_combined
...
levels(d_all_wide$wave) <- WAVES[as.integer(levels(d_all_wide$wave))] # label wave with dates,etc
...
#method1:
# plot mean contact
ggplot(df, aes(x=wave_0, y=all_contact_n))+ 
...
ggplot(df, aes(x=wave_0, y=hh_contact_n))+ 
    stat_summary(geom="ribbon", fun.data=mean_cl_normal, 
                 fun.args=list(conf.int=0.95), fill="lightblue")+
    stat_summary(geom="line", fun=mean, linetype="dashed")+
    stat_summary(geom="point", fun=mean, color="red")+
...
ggplot(df, aes(x=wave_0, y=non_hh_contact_n))+ 
    stat_summary(geom="ribbon", fun.data=mean_cl_normal, 
                 fun.args=list(conf.int=0.95), fill="lightblue")+
    stat_summary(geom="line", fun=mean, linetype="dashed")+
    stat_summary(geom="point", fun=mean, color="red")+
  scale_x_continuous(breaks = seq(1,current_wave,2))+
   coord_cartesian( ylim = c(0, 6))+
  labs(y="Mean_contact_non_hh", x="Wave")+
...
dt <- df %>% select("wave_0","all_contact_n","non_hh_contact_n","hh_contact_n")
...
min_number_waves=29
df <- get_contacts(df,waves=c(1:current_wave),min.waves=min_number_waves)

unique(df$new_id)
rep_n <- unique(df$new_id) #vector store respondents who attend at least min_number_waves
# rep_n <- rep_n[22] # to see individual seperately4


#method1:
# plot mean contact
ggplot(df%>%filter(new_id %in% rep_n), aes(x=wave_0, y=all_contact_n_Q75))+ 
    stat_summary(geom="ribbon", fun.data=mean_cl_normal, 
                 fun.args=list(conf.int=0.95), fill="lightblue")+
    stat_summary(geom="line", fun=mean, linetype="dashed")+
    stat_summary(geom="point", fun=mean, color="red")+
...
# plot mean contact_hh
ggplot(df%>%filter(new_id %in% rep_n), aes(x=wave_0, y=hh_contact_n))+ 
    stat_summary(geom="ribbon", fun.data=mean_cl_normal, 
                 fun.args=list(conf.int=0.95), fill="lightblue")+
    stat_summary(geom="line", fun=mean, linetype="dashed")+
    stat_summary(geom="point", fun=mean, color="red")+
  ...
  ggtitle("Household contact")+
   # theme_bw() +
    theme(axis.text.x = element_text(colour = "black", size = 15, 
          ...
          strip.text = element_text(colour = "black", size = base_size, face = "bold"))
# plot mean contact_non_hh
ggplot(df, aes(x=wave_0, y=non_hh_contact_n_Q75))+ 
    stat_summary(geom="ribbon", fun.data=mean_cl_normal, 
                 fun.args=list(conf.int=0.95), fill="lightblue")+
    stat_summary(geom="line", fun=mean, linetype="dashed")+
    stat_summary(geom="point", fun=mean, color="red")+
...
# all contact in one plot
dt <- df%>%filter(new_id %in% rep_n) %>% select("wave_0","all_contact_n_Q75","non_hh_contact_n_Q75","hh_contact_n")
...
# dt_outside_all_c <- dt_outside_all
# 
...
# dt_outside_all_c$age_group[dt_outside_all_c$age_group=="Don’t know"|dt_outside_all_c$age_group=="Prefer not to answer"] <- NA
...
# combine group contact (individualization) into non_hh contact
keeps <- c("new_id","wave", "age_group")
dt_outside_all_c <- dt_outside_all[ , keeps, drop = FALSE]
colnames(dt_outside_all_c) <- c("new_id","wave_0", "age_group") # rename wave to wave_0 to be in line with fowlling codes
...
colnames(dt_hh_all_c ) <- c("new_id","wave_0", "age_group") # rename wave to wave_0 to be in line with fowlling codes
...
survey_object_den1  <- survey(part_matrix, contact_matrix)
...
#Plot
melt_cm_den1 <- reshape2::melt(data_cm_den1)
...
legend_matrix<- ggplot(melt_cm_den1, aes(contact.age.group, Var1)) +
  ...
  scale_fill_gradient2(low = "#fff5f0",
                       # mid = "white",
                       high = "#e31a1c", limits = c(0,6), midpoint = 0.5) +
  guides(fill = guide_colourbar(barwidth = 0.5,
