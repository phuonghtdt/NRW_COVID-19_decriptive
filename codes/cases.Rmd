...
u_report_w1 <- 0.78
u_report_w2 <- 0.26
cutoff_day <- as.Date("2020-10-10") # cutoff day bwt 2 scale
# cutoff_day <- as.Date("2020-08-16") # to have a smoother , but need motivation for this?
...
source(paste0(path_data_code,"/linelist.R"),encoding="UTF-8")
#(1.1) rki data:https://npgeo-corona-npgeo-de.hub.arcgis.com/datasets/a99afefd425843
source(paste0(path_data_code,"/covid19_rki.R"),encoding="UTF-8")

#(2) age distribution in NRW
source(paste0(path_data_code,"/population.R"),encoding="UTF-8")

#(3) hospitalization
source(paste0(path_data_code,"/hospitalisation.R"),encoding="UTF-8")

#(4)Whole Germany_JHU data # up to date data
source(paste0(path_data_code,"/covid19_jhu.R"),encoding="UTF-8")

# (5) NUTS_3##############################
source(paste0(path_data_code,"/nuts3.R"),encoding="UTF-8")

#(6) Deprivation upto 2015
source(paste0(path_data_code,"/gisd.R"),encoding="UTF-8")

#(7) Regulations
source(paste0(path_data_code,"/regulation.R"),encoding="UTF-8")
 
#(8) vaccinations in NRW
source(paste0(path_data_code,"/vaccination.R"),encoding="UTF-8")
...
under_report_w2 <- 1- report_w2 # 26%
under_report_w2
...
p3 <- dummy %>%
  ...
  scale_fill_manual(values=palette, name="Age group") +
   labs(y="",x="")+
  ggtitle(" Hospitalisation attributed to COVID-19 in NRW_weekly report")+
 scale_x_date(date_labels="%Y-%m-%d", date_breaks = "14 days", expand = c(0, 0.5)) +
